...
class ReportService extends BaseService
{
    ...
    public function getCashSummary($reportType, $date, $month, $year, $userId = null, $role = 'super_admin')
    {
        ...
        $summary['remaining_cash'] = (($summary['cash_sales'] ?? 0) + ($summary['wasel_cash'] ?? 0)) - (($summary['total_cash_expenses'] ?? 0) + ($summary['total_admin_cash_expenses'] ?? 0) + ($summary['total_cash_refunds'] ?? 0) + (($summary['total_compensations'] ?? 0) - ($summary['total_transfer_compensations'] ?? 0)) + ($summary['deposits_yer'] ?? 0));
        $summary['remaining_transfer'] = (($summary['transfer_sales'] ?? 0) + ($summary['wasel_transfer'] ?? 0)) - (($summary['total_transfer_expenses'] ?? 0) + ($summary['total_admin_transfer_expenses'] ?? 0) + ($summary['total_transfer_refunds'] ?? 0) + ($summary['total_transfer_compensations'] ?? 0));
